                ...
                try {
                  ...
                  //  login.put( "username", usernameEditText.getText().toString().trim() );
                   // login.put( "password", passwordEditText.getText().toString().trim() );
                    login.put(username, "salesapi");
                    login.put(password, "123456");

                    progressBar.setVisibility( View.VISIBLE );
                   LoginApplication.apiManager.getUsers( login, new Callback<Users>() {


                        @Override
                        public void onResponse(Call<Users> call, Response<Users> response) {
                            progressBar.setVisibility( View.GONE );
                            Users responseUser = response.body();
                            if (response.isSuccessful() && responseUser != null) {
                                Toast.makeText( LoginActivity.this,
                                        ...
                                        Toast.LENGTH_LONG )
                                        .show();
                                token = responseUser.getAcceesstoken();
